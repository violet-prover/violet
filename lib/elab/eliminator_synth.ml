...
let eliminator_motive_type ~loc ~name ~params ~deps ~ind_ty =
  let good_deps = rename_tele deps in
  let final_ty =
    Surface.apply_tele (at loc (Surface.Var [ name ])) (params @ good_deps)
  in
...
let eliminator_case
      ...
      ~deps
      ~ind_ty
      (ctor : Surface.pretype Surface.sbinder)
    ...
    List.concat_map
      (fun (bind : Surface.pretype Surface.sbinder) ->
         ...
         then (
           ...
           [ bind
           ; { Surface.name =
                 sn loc (Named ("ih-" ^ Name.to_string bind.name.Surface.value))
             ; bound =
                 Surface.apply
...
let%expect_test "`data List (A : U) : U`, the generated eliminator will rely on A" =
  ...
  [%expect
    {|
    [{ Surface.name = (Violet_kernel.Syntax.Named "A"); bound = U;
       implicit = false }
      ]
    |}]
;;
...
let%expect_test "Vec motive" =
  let result =
    eliminator_motive_type
      ...
      ~deps:
        [ { Surface.name = dn Anon; bound = d (Surface.Var [ "Nat" ]); implicit = false }
        ]
      ~ind_ty:(d (Surface.Var [ "U" ]))
  in
  print_string @@ [%show: Surface.pretype] result;
  [%expect {| Π(_0 : Nat) -> Π(_ : ((Vec A) _0)) -> U |}]
;;
...
let%expect_test "Vec case cons" =
  let result =
    eliminator_case
      ...
      ; bound =
          Surface.pi
            [ { Surface.name = dn (Named "k")
              ; bound = d (Surface.Var [ "Nat" ])
              ; implicit = true
              }
            ; { Surface.name = dn (Named "x")
