...
let id_app x y : Surface.preterm =
  Surface.apply (Surface.Var [ "Id" ]) [ Surface.Var [ x ]; Surface.Var [ y ] ]
;;

let diagonal_type ~(xs : string list) ~(ys : string list) : Surface.preterm =
  let rec build xs ys =
    match xs, ys with
    | [], [] ->
      Surface.apply
        (Surface.Var [ "Id" ])
        [ Surface.Var [ "Empty" ]; Surface.Var [ "Empty" ] ]
    | [ x ], [ y ] -> id_app x y
    | x :: xs', y :: ys' ->
      Surface.apply
        (Surface.Var [ "Sigma" ])
...
let diagonal_witness (n : int) : Surface.preterm =
  let rec build n =
    if n = 0
    then Surface.Var [ "Id"; "refl" ]
    else if n = 1
    then Surface.Var [ "Id"; "refl" ]
    else Surface.RecordLit [ "fst", Surface.Var [ "Id"; "refl" ]; "snd", build (n - 1) ]
  in
  build n
;;

let no_confusion_type_def ~(name : string) ~(ctors : Surface.pretype binder list)
  : Surface.pretype * Surface.preterm
  =
  ...
  let make_outer_case (c : Surface.pretype binder) : Surface.preterm =
    let outer_lams, outer_fields =
      case_arg_lambda_binders ~prefix:"x_" ~ind_name:name c
    in
    let make_inner_case (cj : Surface.pretype binder) : Surface.preterm =
      let inner_lams, inner_fields =
      ...
      in
      Surface.lambda inner_lams body
    in
    let inner_elim =
      Surface.apply
        (Surface.Var [ name; "elim" ])
        ([ Surface.Var [ "__y" ]; const_universe_motive ] @ List.map make_inner_case ctors)
    in
    Surface.lambda outer_lams inner_elim
  in
  let outer_elim =
    Surface.apply
      (Surface.Var [ name; "elim" ])
      ([ Surface.Var [ "__x" ]; const_universe_motive ] @ List.map make_outer_case ctors)
  in
  let body = Surface.lambda [ "__x"; "__y" ] outer_elim in
  typ, body
;;

let no_confusion_def ~(name : string) ~(ctors : Surface.pretype binder list)
  : Surface.pretype * Surface.preterm
  =
  let id_xy =
    Surface.apply (Surface.Var [ "Id" ]) [ Surface.Var [ "__x" ]; Surface.Var [ "__y" ] ]
  in
  let nct_xy =
    Surface.apply
      (Surface.Var [ name; "no-confusion-type" ])
      [ Surface.Var [ "__x" ]; Surface.Var [ "__y" ] ]
  in
  let typ : Surface.pretype =
    Surface.pi
      [ { name = Named "__x"; bound = Surface.Var [ name ]; implicit = false }
  ...
  let diag_motive =
    Surface.Lambda
      ...
      ; bound =
          Surface.apply
            (Surface.Var [ name; "no-confusion-type" ])
            [ Surface.Var [ "x_" ]; Surface.Var [ "x_" ] ]
      ; implicit = false
      }
  in
  let make_diag_case (c : Surface.pretype binder) : Surface.preterm =
    let lams, fields = case_arg_lambda_binders ~prefix:"x_" ~ind_name:name c in
    Surface.lambda lams (diagonal_witness (List.length fields))
  in
  let diag_witness =
    Surface.apply
      (Surface.Var [ name; "elim" ])
      ([ Surface.Var [ "__x" ]; diag_motive ] @ List.map make_diag_case ctors)
  in
  let subst_motive =
    Surface.Lambda
      { name = Named "__y'"
