...
let rec env_nth (env : value bwd) (i : int) : value =
  ...
  | Emp, _ -> Reporter.fatalf Elab_error "LocalVar index %d out of range (env empty)" i
;;
...
and vapp_locals (v : value) (env : value bwd) (n : int) : value =
  ...
  if n > env_size
  then
    Reporter.fatalf Elab_error "vapp_locals: n=%d but env has only %d entries" n env_size
  else begin
    (* env_nth_from_left env k = env 從左數第 k 個 *)
    let rec env_nth_from_left env k =
      match env with
      | Emp -> Reporter.fatalf Elab_error "vapp_locals: index %d out of range" k
      | Snoc (rest, top) ->
